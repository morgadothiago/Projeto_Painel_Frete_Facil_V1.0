...
type Tab = "fretes" | "entregas" | "performance";
...
function buildMonthlyFromDaily(
  ...
  months.forEach(m => (counts[`${m.year}-${m.month}`] = 0));
  ...
  return months.map(m => {
    const novas = counts[`${m.year}-${m.month}`] ?? 0;
    running += novas;
    return { mes: m.label, novas, total: running };
  });
}

type Props = { stats?: CompanyDashboardStats | null };

export function CompanyChart({ stats }: Props) {
  const [active, setActive] = useState<Tab>("entregas");
  const meta = TAB_META[active];
  const color = meta.color;

  const overview = stats?.overview;
  const deliveriesMonthly = useMemo(
    () => buildMonthlyFromDaily(stats?.deliveriesLast30Days ?? []),
    [stats],
  );
  ...
  const chartData = useMemo(() => {
    if (active === "entregas") {
      return {
        monthly: deliveriesMonthly,
        status: [
          { label: "Entregues", qtd: overview?.completed ?? 0 },
          { label: "Em Curso", qtd: overview?.inProgress ?? 0 },
          { label: "Pendentes", qtd: overview?.pending ?? 0 },
        ],
      };
    }
    if (active === "fretes") {
      return {
        monthly: deliveriesMonthly,
        status: [
          { label: "Ativos", qtd: (overview?.accepted ?? 0) + (overview?.inProgress ?? 0) },
          { label: "Concluídos", qtd: overview?.completed ?? 0 },
          { label: "Cancelados", qtd: overview?.cancelled ?? 0 },
        ],
      };
    }
    // performance
    const total = (overview?.completed ?? 0) + (overview?.cancelled ?? 0);
    const successRate = total > 0 ? Math.round(((overview?.completed ?? 0) / total) * 100) : 0;
    return {
      monthly: deliveriesMonthly,
      status: [
        { label: "Sucesso", qtd: successRate },
        { label: "Cancelados", qtd: total > 0 ? Math.round(((overview?.cancelled ?? 0) / total) * 100) : 0 },
      ],
    };
  }, [active, deliveriesMonthly, overview]);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          {(["fretes", "entregas", "performance"] as Tab[]).map((key) => {
